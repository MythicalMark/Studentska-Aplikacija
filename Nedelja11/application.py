...
class StudentGradeApp(ctk.CTk):  
    ...
    def create_widgets(self):  
        ...
        ctk.CTkLabel(tab_grades,text="Grade:").grid(row=1,column=0,padx=10,pady=5,sticky="e")
        ctk.CTkEntry(tab_grades,textvariable=self.grade).grid(row=1,column=1,padx=10,pady=5,sticky="ew")
        ctk.CTkButton(tab_grades,text="Add Grade",command="").grid(row=2,column=0,columnspan=2,padx=10,pady=5)
        ...
        ctk.CTkButton(tab_grades,text="Filter",command="").grid(row=4,column=0,columnspan=2,padx=10,pady=5)
        ctk.CTkButton(tab_grades,text="Update Grade",command="").grid(row=5,column=0,padx=10,pady=5)
        ctk.CTkButton(tab_grades,text="Delete Grade",command="").grid(row=5,column=1,padx=10,pady=5)
    ...
    def add_student(self):
        if self.name.get() and self.index_number.get():
            try:
                ...
                self.refresh_data()
                messagebox.showinfo("Successfully added Student")
                self.name.set("")
